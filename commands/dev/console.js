...
class Command extends Message.Event {
	...
	call(message) {
		...
		if (args[0] == "stop") {
			if (this.endSession(message)) {
				//do nothing?
			} else message.crep(`No session going on!`);
			return;
		}
		const time = args[0]
		...
		if (this.startSession(time, message)) {
			const collector = new Discord.MessageCollector(
				message.channel,
				(u) => u.author.id == message.uid && this.inSession,
				{ time: time }
			);
			collector.on("collect", (msg) => {
				...
				try {
					...
					if (error) {
						...
						message.err({
							content: `Could not evaluate! Please check logs! \n[${err.code}]-${err.info.id}`,
						});
					}
				}
			});
		} else return message.crep(`Already a session going on!`);
	}
	...
	endSession(message) {
		if (this.inSession) {
			...
			this.inSession = false;
			message.crep(
				`Ended console session! **Session ID: ${
